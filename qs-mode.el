...
(defvar qs-c-index 0)  ;;   0 = cell Name
...
(define-key qs-map [kp-0] 'qs-keypad-xlate)
(define-key qs-map [kp-1] 'qs-keypad-xlate)
(define-key qs-map [kp-2] 'qs-keypad-xlate)
(define-key qs-map [kp-3] 'qs-keypad-xlate)
(define-key qs-map [kp-4] 'qs-keypad-xlate)
(define-key qs-map [kp-5] 'qs-keypad-xlate)
(define-key qs-map [kp-6] 'qs-keypad-xlate)
(define-key qs-map [kp-7] 'qs-keypad-xlate)
(define-key qs-map [kp-8] 'qs-keypad-xlate)
(define-key qs-map [kp-9] 'qs-keypad-xlate)
...
(defun qs-keypad-xlate () "Translate keypad keystrokes to numbers" (interactive)
       (let* ((kps  [ kp-0 kp-1 kp-2 kp-3 kp-4 kp-5 kp-6 kp-7 kp-8 kp-9])
              (i 0))
         (while (< i (length kps))
           (if (equal (aref kps i) last-input-event)
               (setq i (+ i  47 )) ) ;; (- (string-to-char "0") 1)
           (setq i (+ 1 i)) )
         (setq last-input-event i)
         (qs-edit-cell)))
...
(defun qs-copy-area () "copy current cell or marked range into the copy buffer (if you paste in another window it pastes in latex format" (interactive)
       (let* ((start-col (if qs-mark-cell (if (< qs-cur-col (elt qs-mark-cell 0)) qs-cur-col (elt qs-mark-cell 0)) qs-cur-col))
              (col-len (if qs-mark-cell (- (+ 1 qs-cur-col (car qs-mark-cell)) (* 2 start-col)) 1))
              (start-row (if qs-mark-cell (if (< qs-cur-row (elt qs-mark-cell 1)) qs-cur-row (elt qs-mark-cell 1)) qs-cur-row))
              (row-len (if qs-mark-cell (- (+ 1 qs-cur-row (elt qs-mark-cell 1)) (* 2 start-row)) 1))
              (latex-start (concat "\\begin{tabular}{ |" (string-join (make-list col-len "c") "|")  "| }\n\\hline\n"))
              (latex-end "\\hline\n\\end{tabular}\n")
              (ltx ())
              (rows ()))
         (dotimes (y row-len)
           (let ((lcols ())
                 (ccols ()))
             (dotimes (x col-len)
               (let* ((this-cell  (qs-rowcol-to-index (+ x start-col) (+ y start-row)))
                      (m (avl-tree-member qs-data this-cell))
                      )
                 (setq ccols (append ccols (list m)))
                 (setq lcols (append lcols (list (if m (aref m qs-c-fmtd) "0"))))
                 ))
             (setq rows (append rows (list ccols)))
             (setq ltx (append ltx (list (string-join lcols " & "))))
             )
           )
...
(defun qs-screen-to-col (scol) "get the spreadsheet column from the screen column"
       (let ((acc qs-row-padding)
             (col 0)
             (x (+ scol (window-hscroll)))
             )

         (while (<= (+ acc (elt qs-col-widths col)) x)
